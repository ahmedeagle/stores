...
//			'status_id' => 'required|in:0,1,2',
	...
	public function search(Request $request)
	{
		...
		if ($request->has('id')) {
			...
			array_push($conditions, ['rejectedorders_delivery.status', '=', $status_id]);
		}

		if ($request->has('order_id')) {

			array_push($conditions, ['orders_headers.order_id', '=', $order_id]);
		}
		...
		if (!empty($conditions)) {
			...
			$orders = DB::table('orders_headers')
				...
				->join('users', 'users.user_id', '=', 'orders_headers.user_id')
				->select('orders_headers.order_id', 'orders_headers.order_code', 'orders_headers.total_value', 'rejectedorders_delivery.status', 'rejectedorders_delivery.delivery_id')
				->where($conditions)
				->get();
	...
	//check if this orders if refused before by login delivery
	protected function checknewOrdersRefuseByDelivery($order_id, $delivery_id)
	{

		$refusedBefore = DB::table('rejectedorders_delivery')->where([
			['order_id', '=', $order_id],
			['delivery_id', '=', $delivery_id],
			['status', '=', 0],
		])->first();
